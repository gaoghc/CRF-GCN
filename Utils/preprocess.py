...
def load_data_supervised(dataset_str):
    """
    ...
    objects = []
    for i in range(len(names)):
        with open("/home/ghc/Documents/CRF-GCN/data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    ...
    x, y, tx, ty, allx, ally, graph = tuple(objects)
    test_idx_reorder = parse_index_file("/home/ghc/Documents/CRF-GCN/data/ind.{}.test.index".format(dataset_str))
    test_idx_range = np.sort(test_idx_reorder)
    ...
    # rnd = np.random.permutation(num_samples)
    # num_train = int(num_samples * 0.6)
    # num_val = int(num_samples * 0.2)
    # idx_train = rnd[:num_train]
    # idx_val = rnd[num_train:num_train+num_val]
    # idx_test = rnd[num_train+num_val:]

    # idx_val = rnd[num_train:]
    # idx_test = rnd[num_train:]
    ...
    # print(np.sum(y_train[train_mask, :], 1))
    # print(y_train[train_mask, :])
